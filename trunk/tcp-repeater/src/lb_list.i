...
__u32 round_robin_scheduling(struct lb_struct *lb_hlist,__u32 cur,struct message *msg);




__u32 srcaddr_hash_scheduling(struct lb_struct *lb_hlist,in_addr_t srcaddr,__u32 hash_size,struct message *msg);
...
init_lb_hlist(in_addr_t node[],__u32 num){
    ...
    int i;
    for(i = 0;i<num;i++){
        lb_hlist[i].state = 0X0008;
        lb_hlist[i].count = 0;
        lb_hlist[i].ipaddr = node[i];
        lb_hlist[i].connfd = -1;
        Pthread_mutex_init(&(lb_hlist[i].lb_lock),((void *)0));
    }
    return lb_hlist;
}
...
__u32
round_robin_scheduling(struct lb_struct *lb_hlist,__u32 cur,struct message *msg){
    struct sockaddr_in servaddr;
    bzero(&servaddr,sizeof(servaddr));
    Pthread_mutex_lock(&(lb_hlist[cur].lb_lock));
    if(lb_hlist[cur].connfd ==-1){
 servaddr.sin_family = 2;
 servaddr.sin_port = htons(16665);
 servaddr.sin_addr.s_addr = lb_hlist[cur].ipaddr;
 lb_hlist[cur].connfd = Socket(2,SOCK_STREAM,0);
...
__u32
srcaddr_hash_scheduling(struct lb_struct * lb_hlist,in_addr_t srcaddr,__u32 hash_size,struct message *msg){
    __u32 cur;
...
get_heartbeat(struct lb_struct *lb_hlist,__u32 id,__u32 hash_size){
    ...
    if(lb_hlist[id].state ==0X0001){
        lb_hlist[id].count = 0;
        Pthread_mutex_unlock(&(lb_hlist[id].lb_lock));
    }else if(lb_hlist[id].state == 0X0002){
        Pthread_mutex_unlock(&(lb_hlist[id].lb_lock));
        pthread_t tid;
        Pthread_create(&tid,((void *)0),&fault_query,(void*)(lb_hlist+id));
    }else{
        Pthread_mutex_unlock(&(lb_hlist[id].lb_lock));
    }
}
    ...
    switch(msg->msgtype){
        case 0X0002:
            if(msg->state == 0X0008){
                ...
                Pthread_mutex_lock(&(lb_hlist[i].lb_lock));
                lb_hlist[i].state = 0X0001;
                Pthread_mutex_unlock(&(lb_hlist[i].lb_lock));
     }else if(msg->state ==0X0002){
                Pthread_mutex_lock(&(lb_hlist[i].lb_lock));
                lb_hlist[i].state = 0X0001;
                lb_hlist[i].count = 0;
                Pthread_mutex_unlock(&(lb_hlist[i].lb_lock));
